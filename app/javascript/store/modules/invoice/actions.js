export default {
  ...
  findDiscount({ state, commit }) {
    ...
    if (
      ...
      let discount =
        (tempFooter[0].discountPercentage / 100) * tempFooter[0].subTotal;
      tempFooter[0].discount = discount.toFixed(2);
      tempFooter[0].newsubTotal =
        tempFooter[0].subTotal - tempFooter[0].discount;
    }
  ...
  findcgst({ state, commit }) {
    ...
    if (
      (state.cdetails[0].state_code == state.cldetails[0].state_code &&
        state.cdetails[0].gst_no &&
        state.cldetails[0].gst_no) ||
      (state.cldetails[0].state_code == 32 && !state.cldetails[0].gst_no)
    ) {
  ...
  generatePdf({ state }) {
    if (validatePDF(state)) {
      ...
      let url = `/invoice/invoice.pdf?cdetails=${JSON.stringify(
        state.cdetails
      )}&cldetails=${JSON.stringify(state.cldetails)}&items=${JSON.stringify(
        state.items
      )}&text1=${JSON.stringify(state.text1)}&text2=${JSON.stringify(
        state.text2
      )}&footer=${JSON.stringify(state.footer)}&status=${JSON.stringify(
        state.status
      )}`;
      if (is_chrome) {
        ...
        window.open(
          `/invoice/invoice.pdf?cdetails=${JSON.stringify(
            state.cdetails
          )}&cldetails=${JSON.stringify(
            state.cldetails
          )}&items=${JSON.stringify(state.items)}&text1=${JSON.stringify(
            state.text1
          )}&text2=${JSON.stringify(state.text2)}&footer=${JSON.stringify(
            state.footer
